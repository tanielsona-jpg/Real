...
export default function CanelaDeFogo({ livro, capitulo, textoCapitulo }) {
  ...
  async function enviar() {
    ...
    setCarregando(true);
    try {
      const result = await chamarIA({ pergunta: p, livro, capitulo, textoCapitulo });
      setHistorico(h => [...h, { de: "canela", texto: result.data.resposta }]);
    } catch (err) {
      setHistorico(h => [...h, {
        de: "canela",
        ...
        <div className="canela-panel">
          <div className="canela-panel__header">
            <div className="canela-panel__title">
              <span className="canela-panel__icon">fogo</span>
              <span>Canela de Fogo</span>
            </div>
            <div className="canela-panel__context">{livro} {capitulo}</div>
          </div>
          <div className="canela-panel__messages">
            {historico.length === 0 && (
              <div className="canela-msg canela-msg--canela canela-msg--intro">
                Ola! Pode me perguntar qualquer coisa sobre {livro} {capitulo}. Estou aqui para ajudar na sua leitura.
              </div>
            )}
            {historico.map((msg, i) => (
              <div
